    		...
    		if(GUILayout.Button(testButtonText)){
    			...
    			if(EditorSceneManager.GetActiveScene().Equals(testScenePath) && EditorApplication.isPlaying){
    				...
    				if(!EditorSceneManager.GetActiveScene().Equals(testScenePath)){
                        ...
                        {
                            UnityEngine.SceneManagement.Scene scene = EditorSceneManager.OpenScene(testScenePath, OpenSceneMode.Single);

                            if (!scene.isLoaded){
                                UnityEngine.SceneManagement.Scene newScene = EditorSceneManager.NewScene(NewSceneSetup.DefaultGameObjects);
